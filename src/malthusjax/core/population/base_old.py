...
class AbstractPopulation(ABC, Generic[T]):
    ...
    def __getitem__(self, index: int) -> T:
        #TODO: fix this method to return a solution by index
        """Get a solution by index."""
        solutions = self.get_solutions()
        print(f"Index: {index}, Solutions Length: {len(solutions)}")
    ...
    def get_distance_matrix_function_jit(self) -> Callable[[jax.Array, jax.Array], float]:
        ...
        """
        distance_function = self.solution_class(self.genome_init_params).genome.get_distance_jit()
        @jax.jit
    ...
    def from_array_of_indexes_jit(self) -> Callable[[jnp.ndarray], jnp.ndarray]:
        ...
        Returns:
            A callable that takes a JAX array of indexes and a stack, returning the selected solutions.
        """
        def func(stack: jax.Array, indexes: jax.Array) -> jnp.ndarray:
            population_indexes = jax.vmap(lambda i: stack[i] if i < stack.shape[0] else None)(indexes)
            return population_indexes

        return jax.jit(func)
    
    def get_autovalidation_function_jit(self) -> Callable[[jax.Array], jax.Array]:
        """Get JIT-compiled function that turns invalid genomes into valid ones."""
        auto_correction_function = self.solution_class(self.genome_init_params).genome.get_autovalidation_jit()
