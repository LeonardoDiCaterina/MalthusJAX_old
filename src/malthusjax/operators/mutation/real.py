...
class BallMutation(AbstractMutation[Population]):
    """Ball mutation operator for real-valued genomes.
    
    Builds and returns a JIT-compiled function that perturbs values in real-valued genomes
    with a specified probability.
    """
    
    def __init__(self, mutation_rate: float = 0.1, mutation_strength: float = 0.1) -> None:
        """Initialize ball mutation operator.
        
        Args:
            mutation_rate: Probability of perturbing each value.
            mutation_strength: Maximum perturbation strength.
        """
    ...
    def _create_mutation_function(self) -> Callable:
        """Create the core ball mutation function.
        
        Returns:
            Pure function for ball mutation that can be JIT-compiled.
        """
        @jax.jit
        def ball_mutation(genome_data, random_key, mutation_rate):
            """Core ball mutation function.

            Args:
                genome_data: Real-valued genome data array.
            ...
            key1, key2 = jar.split(random_key)
            # Create mutation mask with probability mutation_rate
            mutation_mask = jar.bernoulli(key1, mutation_rate, genome_data.shape)
            # Apply mutation by perturbing values where mask is True
            perturbation = jax.random.uniform(key2, shape=genome_data.shape, minval=-self.mutation_strength, maxval=self.mutation_strength)
            return jnp.where(mutation_mask, genome_data + perturbation, genome_data)
